...
// Reschedule 更新观测的实际采集时间，改期后定位批次自动重新计算。
func (r *ObservationRepository) Reschedule(ctx context.Context, id uint, observedAt time.Time, actor Actor) (model.BearingObservation, error) {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if before.ObservedAt.UTC().Equal(observedAt.UTC()) {
			return api.NewError(409, "OBSERVATION_TIME_UNCHANGED", "改期时间与原采集时间相同，无需更新")
		}
		result := tx.Model(&model.BearingObservation{}).Where("id = ?", id).Update("observed_at", observedAt)
		if result.Error != nil {
